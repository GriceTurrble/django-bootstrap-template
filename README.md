...
The `base_objects` app is a standard Django app included with the site template. This app is intended to contain [abstract models](https://docs.djangoproject.com/en/3.0/topics/db/models/#abstract-base-classes), utility methods, and other "common" objects you may use elsewhere in your site. Think of it as a root directory for custom Django objects.
...
- `base_objects` includes a barebones [AppConfig](https://docs.djangoproject.com/en/3.0/ref/applications/), `BaseObjectsConfig`. You can find this in `base_objects/apps.py`.
  - An app's AppConfig is a good place to [connect model signals](https://docs.djangoproject.com/en/3.0/topics/signals/#connecting-receiver-functions) by importing an app's `signals.py` module in that app's `AppConfig.ready` method. This is typically cleaner than defining signal receivers in `models.py`.
